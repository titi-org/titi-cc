...
// Iteration 17: Driving - fold detection
 ...
 run_obj_t twice_step ( run_obj_t self_void , run_obj_t twice_arg_void );
  return ((int) run_step ( (run_obj_t)program_step , (run_obj_t)twice_step , (run_obj_t)(x) )) ;
   ...
   int quad ( int x ) {
 run_obj_t quad_step ( run_obj_t self_void , run_obj_t quad_arg_void );
  return ((int) run_step ( (run_obj_t)program_step , (run_obj_t)quad_step , (run_obj_t)(x) )) ;
}
...
/* SPECIALIZED CODE START */
 ...
 run_obj_t quad_step ( run_obj_t self_void , run_obj_t quad_arg_void ) { 
  run_cc_start_quad_step: ;
  run_obj_t (*self)(run_obj_t, run_obj_t) = (run_obj_t(*)(run_obj_t, run_obj_t))self_void;
  __auto_type x = quad_arg_void;
