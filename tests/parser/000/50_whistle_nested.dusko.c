...
// Iteration 15: Supercompilation whistle - nested recursion
   int nested ( int n ) {
 run_obj_t nested_step ( run_obj_t self_void , run_obj_t nested_arg_void );
  return ((int) run_step ( (run_obj_t)kleene_step , (run_obj_t)nested_step , (run_obj_t)(n) )) ;
}
...
/* SPECIALIZED CODE START */
 run_obj_t nested_step ( run_obj_t self_void , run_obj_t nested_arg_void ) { 
  run_cc_start_nested_step: ;
  run_obj_t (*self)(run_obj_t, run_obj_t) = (run_obj_t(*)(run_obj_t, run_obj_t))self_void;
  __auto_type n = nested_arg_void;
