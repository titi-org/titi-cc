...
/* SPECIALIZED CODE START */
 ...
 run_obj_t test_axioms_step ( run_obj_t self_void , run_obj_t test_axioms_arg_void ) { 
  ...
  /* Eval / Run */
                       RUN_ASSERT ( bool_value ( run_iseq ( eval ( f , x ) , f ( x ) ) ) , "eval(f, x) == f(x)" ) ; 
                           
             RUN_ASSERT ( bool_value ( run_iseq ( run ( program , f , x ) , f ( x ) ) ) , "run(program, f, x) == f(x)" ) ; 

  /* Variadic identity/idempotent */
                          RUN_ASSERT ( bool_value ( run_iseq ( eval ( f , x ) , f ( x ) ) ) , "identity(f, x) == f(x)" ) ; 
                         
             RUN_ASSERT ( bool_value ( run_iseq ( eval ( f , x ) , f ( x ) ) ) , "idempotent(f, x) == f(x)" ) ; 
                                       
             RUN_ASSERT ( bool_value ( run_iseq ( eval ( f , eval ( g , x ) ) , f ( g ( x ) ) ) ) , "identity(f, g, x) == f(g(x))" ) ; 
                                                    
             RUN_ASSERT ( bool_value ( run_iseq ( eval ( f , eval ( g , nat_from_int ( 1 ) ) ) , f ( g ( nat_from_int ( 1 ) ) ) ) ) , "idempotent(f, g, 1) == f(g(1))" ) ; 

  /* Equality and Decidability */
         RUN_ASSERT ( bool_value ( run_iseq ( x , x ) ) , "iseq(x, x) == true" ) ; 
                RUN_ASSERT (  ! bool_value ( run_iseq ( x , nat_succ ( x ) ) )  , "iseq(x, x+1) == false" ) ; 

  /* rho(x) == x if x is a boolean projection (true or false) */
              
             RUN_ASSERT ( bool_value ( run_iseq ( run_rho ( RUN_BOOL_TRUE ) , RUN_BOOL_TRUE ) ) , "rho(true) == true" ) ; 
